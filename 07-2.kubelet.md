...
6gxij2.kbg7o3j6zhs51hjs   23h       2019-04-02T13:44:19+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:k8s-master03
cos6nl.alfkczwhljqewlsj   23h       2019-04-02T13:44:19+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:k8s-master02
et3dom.wftq19bvy3fv94w5   23h       2019-04-02T13:44:18+08:00   authentication,signing   kubelet-bootstrap-token   system:bootstrappers:k8s-master01
```
...
csr-gp6sd   23s   system:bootstrap:et3dom   Pending
csr-prwvx   21s   system:bootstrap:6gxij2   Pending
csr-xrx5s   22s   system:bootstrap:cos6nl   Pending
...
``` bash
$ curl -s --cacert /etc/kubernetes/cert/ca.pem https://172.20.56.224:10250/metrics
Unauthorized

$ curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer 123456" https://172.20.56.224:10250/metrics
Unauthorized
```
...
``` bash
$ # 权限不足的证书；
$ sudo curl -s --cacert /etc/kubernetes/cert/ca.pem --cert /etc/kubernetes/cert/kube-controller-manager.pem --key /etc/kubernetes/cert/kube-controller-manager-key.pem https://172.20.56.224:10250/metrics
Forbidden (user=system:kube-controller-manager, verb=get, resource=nodes, subresource=metrics)

$ # 使用部署 kubectl 命令行工具时创建的、具有最高权限的 admin 证书；
$ sudo curl -s --cacert /etc/kubernetes/cert/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem https://172.20.56.224:10250/metrics|head
...
$ curl -s --cacert /etc/kubernetes/cert/ca.pem -H "Authorization: Bearer ${TOKEN}" https://172.20.56.224:10250/metrics|head
# HELP apiserver_client_certificate_expiration_seconds Distribution of the remaining lifetime on the certificate used to authenticate a request.
# TYPE apiserver_client_certificate_expiration_seconds histogram
apiserver_client_certificate_expiration_seconds_bucket{le="0"} 0
apiserver_client_certificate_expiration_seconds_bucket{le="21600"} 0
...
浏览器访问 https://172.20.56.224:10250/metrics 和 https://172.20.56.224:10250/metrics/cadvisor 分别返回 kublet 和 cadvisor 的 metrics。
...
$ # 使用部署 kubectl 命令行工具时创建的、具有最高权限的 admin 证书；
$ source /opt/k8s/bin/environment.sh
$ sudo curl -sSL -k --cacert /etc/kubernetes/cert/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem ${KUBE_APISERVER}/api/v1/nodes/k8s-master01/proxy/configz | jq \
...
在使用curl请求kube-apiserver的时候，报错，请求的URL为：

```
curl -sSL --cacert /etc/kubernetes/cert/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem ${KUBE_APISERVER}/api/v1/nodes/k8s-master01/proxy/configz | jq '.kubeletconfig|.kind="KubeletConfiguration"|.apiVersion="kubelet.config.k8s.io/v1beta1"'
```
...
```
curl -sSL -k --cacert /etc/kubernetes/cert/ca.pem --cert /opt/k8s/work/admin.pem --key /opt/k8s/work/admin-key.pem ${KUBE_APISERVER}/api/v1/nodes/k8s-master01/proxy/configz | jq '.kubeletconfig|.kind="KubeletConfiguration"|.apiVersion="kubelet.config.k8s.io/v1beta1"'
```
