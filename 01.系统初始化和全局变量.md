...
+ k8s-slave01: 172.20.56.62

本文档中的`etcd`集群、`master`节点、`worker `节点均使用这四台机器。
...
``` bash
cat >> /etc/hosts << EOF
172.20.56.224 k8s-master01	 k8s-master01
172.20.56.234 k8s-master02	 k8s-master02
172.20.56.228 k8s-master03	 k8s-master03
172.20.56.62  k8s-slave01    k8s-slave01
EOF
```
...
如果没有特殊指明，本文档的所有操作**均在 172.20.56.224 k8s-master01 节点上执行**，然后远程分发文件和执行命令，所以需要添加该节点到其它节点的 ssh 信任关系。

设置 k8s-master01 可以无密码登录**所有节点**的 root 账户：
...
scp /etc/sysctl.d/kubernetes.conf root@k8s-master02:/etc/sysctl.d/kubernetes.conf
scp /etc/sysctl.d/kubernetes.conf root@k8s-master03:/etc/sysctl.d/kubernetes.conf
scp /etc/sysctl.d/kubernetes.conf root@k8s-slave01:/etc/sysctl.d/kubernetes.conf
